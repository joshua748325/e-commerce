...
def new_conversation(request, item_pk):
    ...
    if conversations:
        return redirect(reverse('conversation:detail',kwargs={'pk':conversation.first().pk}))
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            return redirect(reverse('item:detail', kwargs={'pk':item_pk}))
    else:
...
def detail(request,pk):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            conversation.save()
            return redirect(reverse('conversation:detail',kwargs={'pk':pk}))
    else:
        form=ConversationMessageForm()
